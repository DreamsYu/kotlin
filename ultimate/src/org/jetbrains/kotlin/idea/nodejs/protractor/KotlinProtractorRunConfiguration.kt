...
class KotlinProtractorRunConfiguration(
    ...
    override fun checkConfiguration() {
        ...
        if (isOptionalConfig) {
            validatePath(false, "test file", runSettings.testFileSystemDependentPath, true)
        ...
        if (!isOptionalConfig || runSettings.configFileSystemDependentPath.isNotBlank()) {
            validatePath(false, "configuration file", runSettings.configFileSystemDependentPath, true)
        }

        val interpreter = runSettings.interpreterRef.resolve(project)
        NodeJsLocalInterpreter.checkForRunConfiguration(interpreter)

        validatePath(true, "protractor package", protractorPackage.systemDependentPath, true)
    }
    ...
    private fun validatePath(shouldBeDirectory: Boolean,
                             name: String,
                             path: String?,
                             shouldBeAbsolute: Boolean
    ) {
        if (StringUtil.isEmptyOrSpaces(path)) {
            throw RuntimeConfigurationError("Unspecified " + name)
        }
        val file = File(path!!)
        if (shouldBeAbsolute && !file.isAbsolute) {
            throw RuntimeConfigurationError("No such " + name)
        }
